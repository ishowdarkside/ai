...
export default function Generator() {
	const { image, setFile, file, positions: { x, y }, selectedBackground, boxRef } = useFileContext();
	...
	async function handleCompose() {
		...
		const resizedProduct = await resizeProduct(formData, productWidth, productHeight);
		...
		mergeImages([ backgroundByte, { src: resizedProduct, x: x * 1.5, y: y * 1.5 } ])
			...
			.catch((err) => {
				// Ovdje se treba ubacit poruka i kad je pun storage
				console.log(err);
			});
	}
	return (
			...
			<div className={styles.panelsWrapper}>
				<div className={styles.promptWrapper}>
					...
					<DndProvider backend={HTML5Backend}>
						...
						>
							<DraggableBox id="box1" left={50} top={50} image={image} isMoving={isMoving} />
						</div>
					</DndProvider>

					<button onClick={() => setIsMoving((curr) => !curr)} className={styles.toggleBtn}>
						{isMoving ? (
				...
				{processedProduct ? (
					<a href={processedProduct} download className={styles.outputLink}>
						...
						<div style={{ backgroundImage: `url(${processedProduct})` }} className={styles.outputWrapper} />
					</a>
				) : (
